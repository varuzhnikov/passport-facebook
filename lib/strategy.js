...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  var_oauth2 = this._oauth2;

  var_oauth2.get(url, accessToken, function (err, body, res) {
    var json;
    ...
    var_oauth2.get('https://graph.facebook.com/v2.2/me/picture?redirect=false&height=400&width=400', accessToken, function (err, body, res) {
      ...
      if (err) {
        if (err.data) {
          try {
            json_picture = JSON.parse(err.data);
          } catch (_) {}
        }
        
        if (json_picture && json_picture.error && typeof json_picture.error == 'object') {
          return done(new FacebookGraphAPIError(json_picture.error.message, json_picture.error.type, json_picture.error.code, json_picture.error.error_subcode));
        }
        return done(new InternalOAuthError('Failed to fetch user picture', err));
      }
      
      try {
        json_picture = JSON.parse(body);
        console.log('json_picture');
        console.dir(json_picture);
      } catch (ex) {
        return done(new Error('Failed to parse user picture'));
      }
      ...
      profile._raw = body;
      profile._json = json;
      profile.json_picture = json_picture
